...
function addByX(x) {
  let newFunc= (num) => {console.log(num + x)}
  return newFunc
}
...
function censor() {
  let store =[];
    ...
    if (a, b != undefined){
      ...
      a.replace(store[0][0], store[0][1]);  
      return a.replace(store[1][0], store[1][1]); 
    }
  }
  return newFunc
}
...
function createSecretHolder(secret) {
  ...
  return {    
    getSecret(){ return sec},
    setSecret(val){ sec = val; return sec }
  }	
}
...
function russianRoulette(num) {
  ...
  return () => {
    if(count === 0) return("reload to play again"); 
    
    if (count === 1) { count--; return("Bang"); }
    count--;
    return("click") 
  }
}
...
function average() {
  ...
  return ((num) => {
    if(num !== undefined){ 
      store.push(num); 
      return store.reduce((a,b) => a + b, 0) / store.length;
  } else {
...
function makeFuncTester(arrOfTests) {
  const store = arrOfTests;
  
  return((cb) => {
    let a1 = store.map((item, i) => { return( [cb(item[0]), item[1]] )})
...
function makeHistory(limit) {
  ...
  return(str => {
    if(str === "undo" && store.length === 0 ) return("nothing to undo");
      else if(str === "undo") return(store.pop() + " undone");
